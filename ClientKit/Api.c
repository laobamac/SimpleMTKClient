...
bool connect_network(const char *ssid, const char *pwd) {
    ...
    while (timeout-- > 0) {
        ...
        sleep(1);
        uint32_t state;
        if (get_80211_state(&state) && state == MTK80211_S_RUN) {
...
{
    if (strcmp(name, "SimpleMtkWlan") && strcmp(name, "TestService")) {
        return false;
    }
    return true;
}

static bool open_service(io_service_t service, io_connect_t *connection_t)
{
    if (!service) {
        return false;
    }
    return IOServiceOpen(service, mach_task_self(), 0, connection_t) == KERN_SUCCESS;
}
...
bool open_adapter(io_connect_t *connection_t)
{
    ...
    CFMutableDictionaryRef serviceDict = IOServiceMatching("SimpleMtkWlan");
    service = IOServiceGetMatchingService(port, serviceDict);
    if (open_service(service, connection_t)) {
        found = true;
    }
    if (service) {
