...
class StatusMenuBase: NSMenu, NSMenuDelegate {
    ...
    private var driverState: mtk_80211_state = MTK80211_S_INIT {
        didSet {
            /** Only allow if network card is enabled or if the network card does not load
             either due to SimpleMtkWlan not loaded or just not able to receive info
            ...
            case MTK80211_S_SCAN:
                /** API does not report bgscan to SimpleMTK. During `MTK80211_S_RUN` the status
                 will never change to `MTK80211_S_SCAN` unless users manually disassociate.
    ...
    let bsdItem = SimpleMTKMenuItem(title: .interfaceName)
    let macItem = SimpleMTKMenuItem(title: .macAddress)
    let driverVerItem = SimpleMTKMenuItem(title: .driverVer)

    let enableLoggingItem = SimpleMTKMenuItem(title: .enableWiFiLog)
    let createReportItem = SimpleMTKMenuItem(title: .createReport)
    let diagnoseItem = SimpleMTKMenuItem(title: .openDiagnostics)
    let hardwareInfoSeparator = NSMenuItem.separator()

    let networkItemListSeparator = NSMenuItem.separator()

    let aboutItem = SimpleMTKMenuItem(title: .aboutSimpleMTK)
    let checkUpdateItem = {
         let item = SimpleMTKMenuItem(title: .checkUpdates)
    ...
    let quitItem = SimpleMTKMenuItem(title: .quitSimpleMTK,
                              action: #selector(clickMenuItem(_:)), keyEquivalent: "q")

    let toggleLaunchItem = SimpleMTKMenuItem(title: .launchLogin,
                                      action: #selector(clickMenuItem(_:)))

    // MARK: - WiFi connected items

    let currentNetworkItem = SimpleMTKMenuItem(highlightable: true)
    let ipAddresssItem = SimpleMTKMenuItem(title: .ipAddr)
    let routerItem = SimpleMTKMenuItem(title: .routerStr)
    let internetItem = SimpleMTKMenuItem(title: .internetStr)
    let securityItem = SimpleMTKMenuItem(title: .securityStr)
    let bssidItem = SimpleMTKMenuItem(title: .bssidStr)
    let channelItem = SimpleMTKMenuItem(title: .channelStr)
    let countryCodeItem = SimpleMTKMenuItem(title: .countryCodeStr)
    let rssiItem = SimpleMTKMenuItem(title: .rssiStr)
    let noiseItem = SimpleMTKMenuItem(title: .noiseStr)
    let txRateItem = SimpleMTKMenuItem(title: .txRateStr)
    let phyModeItem = SimpleMTKMenuItem(title: .phyModeStr)
    let mcsIndexItem = SimpleMTKMenuItem(title: .mcsStr)
    let nssItem = SimpleMTKMenuItem(title: .nssStr)
    ...
    func addNetworkItem(_ item: NSMenuItem = SimpleMTKMenuItem(highlightable: true),
                        insertAt: Int? = nil,
                        hidden: Bool = false,
    ...
    private func getDeviceInfo() {
        DispatchQueue.global(qos: .background).async {
            ...
            var driverVer: String = .unavailable
            ...
            if get_platform_info(&platformInfo) {
                ...
                driverVer = String(cCharArray: platformInfo.driver_info_str)
            }
            ...
            DispatchQueue.main.async {
                if let items = self as? StatusMenuItems {
                    ...
                    items.setValueForItem(self.driverVerItem, value: driverVer)
                }
            }
            ...
            if get_80211_state(&state) && power &&
                (state != MTK80211_S_RUN.rawValue || get_station_info(&stationInfo) != KERN_SUCCESS) {
    ...
    @objc private func updateStatus() {
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                self.driverState = mtk_80211_state(rawValue: status)
                self.updateStationItems()
            }
        }
    }
    ...
    private func getStationInfo() -> StationInfo {
        ...
        guard driverState == MTK80211_S_RUN,
...
private extension String {
    ...
    static let driverVer = NSLocalizedString("Version: ")
    ...
    static let aboutSimpleMTK = NSLocalizedString("About SimpleMTK")
    static let quitSimpleMTK = NSLocalizedString("Quit SimpleMTK")
