...
final class StatusMenuModern: StatusMenuBase, StatusMenuItems {
    ...
    private lazy var otherSectionItem: NSMenuItem = {
        let item = SimpleMTKMenuItem(highlightable: true)
        item.isHidden = true
    ...
    private let manuallyJoinItem = SimpleMTKMenuItem(title: .Modern.joinNetworks)
    private let networkPanelItem = SimpleMTKMenuItem(title: .Modern.wifiSettings)
    ...
    func setupMenu() {
        ...
        [bsdItem, macItem, driverVerItem].forEach {
    ...
    override func addNetworkItem(_ item: NSMenuItem = SimpleMTKMenuItem(highlightable: true),
                                 insertAt: Int? = nil,
                                 hidden: Bool = false,
